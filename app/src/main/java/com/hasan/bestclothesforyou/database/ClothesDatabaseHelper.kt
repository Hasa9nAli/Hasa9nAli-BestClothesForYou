    ...
    fun getAllClothes(): List<ClothesData> {
        ...
        cursor?.use {
            ...
            while (it.moveToNext()) {
                ...
                val weatherDegreeString = cursor.getString(cursor.getColumnIndexOrThrow(WEATHER_DEGREE))
                val weatherDegree = Pair(weatherDegreeString.substringBefore("-").toInt(), weatherDegreeString.substringAfter("-").toInt())
                // Convert byte array to Bitmap
                val imageBitmap = BitmapFactory.decodeByteArray(imageByteArray, 0, imageByteArray.size)
                val clothes = ClothesData(id, name, imageBitmap, weatherDegree, category, seasonType)
